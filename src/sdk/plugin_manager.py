    ...
    async def register_plugin(self, spec: BasePluginSpec):
        async with AsyncClient() as client:
            ...
            if response.status_code == 409:
                logger.warning(
                    f"Plugin {spec.name} is already registered, skipping registration."
                )
                return
            if response.status_code != 201:
                logger.error(
                    f"Failed to register plugin {spec.name} with status code {response.status_code}"
                )
                raise UnexpectedValue(
                    f"Failed to register plugin: {response.status_code}"
                )
